    ...
    <div class="max-w-7xl mx-auto">
      <div class="mb-12 text-center md:text-left flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div class="max-w-2xl">
          ...
          <p class="text-slate-600 text-lg">Kunjungi outlet laundry kami yang tersebar di beberapa titik untuk
            memberikan layanan pencucian terbaik tanpa antre panjang.</p>
        </div>
        <div v-if="activeLocations.length > 0"
          class="inline-flex items-center gap-2 bg-white px-5 py-2.5 rounded-full border border-slate-200 shadow-sm text-sm font-bold text-slate-700">
      ...
      <div v-else>
        <!-- ==== SINGLE LOCATION UI ==== -->
        <div v-if="activeLocations.length === 1"
          class="bg-white rounded-[2rem] p-4 md:p-8 shadow-sm border border-slate-200 flex flex-col lg:flex-row gap-8 items-center group">
          <div class="w-full lg:w-1/2 flex flex-col gap-6 md:px-4">
            ...
            <a :href="`https://maps.google.com/?q=${activeLocations[0].latitude},${activeLocations[0].longitude}`"
              target="_blank"
              class="mt-4 inline-flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-white py-4 px-8 rounded-xl font-bold transition-all shadow-md group-hover:shadow-lg w-full sm:w-auto">
              <span class="material-symbols-outlined">directions</span>
              Dapatkan Rute
            </a>
          </div>
          <div
            class="w-full lg:w-1/2 h-[400px] lg:h-[500px] rounded-[1.5rem] overflow-hidden border border-slate-200 relative bg-slate-100">
        ...
        <div v-else class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div class="lg:col-span-4 flex flex-col gap-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
            <button v-for="loc in activeLocations" :key="loc.id" @click="selectedLocation = loc"
              ...
              <div class="flex items-start gap-4">
                <div class="mt-1 w-10 h-10 rounded-full flex items-center justify-center shrink-0 transition-colors"
                  :class="selectedLocation?.id === loc.id ? 'bg-primary text-white' : 'bg-slate-100 text-slate-500'">
                  <span class="material-symbols-outlined text-xl">location_on</span>
                </div>
                <div>
                  <h4 class="font-bold text-lg mb-1"
                    :class="selectedLocation?.id === loc.id ? 'text-primary' : 'text-slate-900'">{{ loc.name }}</h4>
                  <p class="text-sm line-clamp-2"
                    :class="selectedLocation?.id === loc.id ? 'text-slate-700' : 'text-slate-500'">{{ loc.address }}</p>
                </div>
              </div>
            </button>
          </div>
          ...
          <div class="lg:col-span-8 bg-white p-4 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col">
            <div
              class="h-[400px] lg:h-[500px] rounded-[1.5rem] overflow-hidden bg-slate-100 relative w-full border border-slate-100">
              <iframe v-if="selectedLocation" class="w-full h-full absolute inset-0 transition-opacity duration-500"
            ...
            <div v-if="selectedLocation"
              class="mt-6 px-4 md:px-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
              ...
              <a :href="`https://maps.google.com/?q=${selectedLocation.latitude},${selectedLocation.longitude}`"
                target="_blank"
                class="inline-flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-white py-3 px-6 rounded-xl font-bold transition-all shadow-md mt-2 sm:mt-0 text-sm">
